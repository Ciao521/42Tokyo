...
async function mint() {
  // transaction のデータを作成
  let useraddress = await nftContract.methods.mint(WALLET_ADDRESS).call();
  console.log(useraddress);
  const data = nftContract.methods.mint(WALLET_ADDRESS).encodeABI(); //mintでNFTを発行する際のWALLET_ADDRESSは発行先
