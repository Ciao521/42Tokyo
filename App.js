...
function App() {
    ...
    const mintToken = async () => {
        ...
        const setRentalData = async ()=>{
            let nonce = await web3.eth.getTransactionCount(owner, 'latest');
            let tokenId = Number(tokenId);
            let expire = new Date(expire).getTime() / 1000;
            let rewardAmount = Number(rewardAmount);
            const data = contract.methods.setRentalData(renter, tokenId, expire, rewardAmount).encodeABI();
            let gas = await contract.methods.setRentalData(renter, tokenId, expire, rewardAmount).estimateGas();
